...
def run() -> dict:
    ...
    # --- CANONICAL SQUAD STATE (council v4: API-first, never fiction) --------
    import squad_state
    have_team = False
    sq_state = None
    if cfg.get("team_id"):
        sq_state = squad_state.current(cfg["team_id"])
        have_team = sq_state["source"] != "none" and len(sq_state["element_ids"]) == 15
        state["squad_state"] = dict(source=sq_state["source"], gw=sq_state["gw"],
                                    chip=sq_state.get("chip"), bank=sq_state["bank"],
                                    unresolved=sq_state["unresolved"])

    locked = cfg.get("locked") or []
    banned = cfg.get("banned") or []

    if have_team:
        ids = sq_state["element_ids"]
        bank = sq_state["bank"]
        plans = optimizer.transfer_plan(proj, ids, bank,
                                        free_transfers=sq_state["free_transfers"],
